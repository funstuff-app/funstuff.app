#!/usr/bin/env bash
#
# Deploy funstuff.app landing page to Raspberry Pi
#
...
scp "$SCRIPT_DIR/funstuff-landing.service" "${PI_TARGET}:/tmp/${SERVICE_NAME}.service"
...
# ─────────────────────────────────────────────────────────────────────────────
# Update cloudflared tunnel config to add funstuff.app hostname
# ─────────────────────────────────────────────────────────────────────────────
log_step "Checking cloudflared tunnel config..."

TUNNEL_CONFIG=$(ssh "$PI_TARGET" "cat /home/${PI_USER}/.cloudflared/config.yml")

if echo "$TUNNEL_CONFIG" | grep -q "hostname: funstuff.app"; then
  log_info "funstuff.app already in tunnel config"
else
  log_warn "Adding funstuff.app to cloudflared tunnel config..."

  # Get the credentials filename first
  CREDS_FILE=$(ssh "$PI_TARGET" "basename /home/${PI_USER}/.cloudflared/*.json")

  ssh "$PI_TARGET" "cp /home/${PI_USER}/.cloudflared/config.yml /home/${PI_USER}/.cloudflared/config.yml.bak"
...
ingress:
  - hostname: dustytrails.funstuff.app
    service: http://localhost:8766
  - hostname: funstuff.app
    service: http://localhost:${LANDING_PORT}
  - hostname: www.funstuff.app
    service: http://localhost:${LANDING_PORT}
  - service: http_status:404
CFEOF"
  log_info "Tunnel config updated. Restarting cloudflared..."
  ssh -t "$PI_TARGET" "sudo systemctl restart cloudflared"
  log_info "cloudflared restarted"
fi
...
log_info " Public:   https://funstuff.app/ (after DNS setup)"
log_info ""
log_info " IMPORTANT: You need to add a DNS record in Cloudflare:"
log_info "   funstuff.app → CNAME → <tunnel-id>.cfargotunnel.com"
log_info "   (or use: cloudflared tunnel route dns dustytrails funstuff.app)"
log_info "════════════════════════════════════════════════════════"
