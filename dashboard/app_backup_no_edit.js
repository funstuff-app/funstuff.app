...
function primaryReadingFromPoint(p) {
  ...
  for (const k of order) {
    if (readings[k]) return readings[k];
  }
...
function primaryReadingKeyedFromPoint(p) {
  ...
  for (const k of order) {
    if (readings[k]) {
      return { key: k, value: readings[k].value, color: safeHex(readings[k].color) };
    }
  }
  const keys = Object.keys(readings);
  if (!keys.length) return null;
  const k0 = keys[0];
  const r0 = readings[k0];
  return { key: k0, value: r0?.value, color: safeHex(r0?.color) };
}

function primaryReadingForSensor(s) {
  // Prefer server-provided “worst AQI” selection if present.
  if (s && s.primary_key != null) {
    return {
      key: s.primary_key,
      value: s.primary_value,
      color: safeHex(s.primary_color),
    };
  }
  ...
  const order = ["PM25", "PM2.5", "PM10", "OZNE", "Ozone"];
  for (const k of order) {
    if (readings[k]) return { key: k, value: readings[k].value, color: safeHex(readings[k].color) };
  }
  const keys = Object.keys(readings);
  if (!keys.length) return { key: null, value: null, color: "#ffffff" };
  const k0 = keys[0];
  return { key: k0, value: readings[k0].value, color: safeHex(readings[k0].color) };
}
...
class MapView {
  constructor(tilesCanvas, overlayCanvas) {
    ...
    this._pbDrag = null; // { id, startedAtMs, lastClient:{x,y} }
  ...
  onMouseDown(e) {
    ...
    // DVR: dragging a mobile marker scrubs playback time along its historical path.
    if (this.playbackMode) {
      const nowMs = performance.now();
      const hit = this._hitTestMobileAtClientXY(e.clientX, e.clientY, nowMs);
      if (hit && hit.id != null) {
        try { e.preventDefault(); e.stopPropagation(); } catch {}
        this._pbDrag = { id: String(hit.id), startedAtMs: nowMs, lastClient: { x: e.clientX, y: e.clientY } };
        // Stop playback while scrubbing.
        this.setPlaybackPlaying(false);
        // Treat as a drag so onClick does not toggle selection.
        this._mouseDragMoved = false;
        this._scrubPlaybackTimeForMobileAtClientXY(hit, e.clientX, e.clientY);
  ...
  onMouseMove(e) {
    if (this._pbDrag && this.playbackMode) {
      ...
      if (m) {
        this._scrubPlaybackTimeForMobileAtClientXY(m, e.clientX, e.clientY);
        this.drawOverlay(this.lastState);
      }
      return;
    }
  ...
  _scrubPlaybackTimeForMobileAtClientXY(mobile, clientX, clientY) {
    ...
    const scan = (i0, i1, step) => {
      ...
      for (let i = start; i <= end; i += step) {
        ...
        if (hit.d2 < best.d2) best = { i, t: hit.t, d2: hit.d2 };
      }
    };

    scan(0, n - 2, stride);
    const win = Math.max(20, stride * 6);
    ...
    if (isFinite(bounds.minMs) && isFinite(bounds.maxMs)) {
      this.setPlaybackTimeMs(clamp(tMs, bounds.minMs, bounds.maxMs));
    } else {
  ...
  _traceSampleForMobile(m, nowMs) {
    ...
    // If the backend says this vehicle is idle/ghosted, do NOT animate a replay route.
    // Trace mode is meant to visualize motion; idling GPS jitter should render as stationary.
    if (m && m.ghosted) {
      ...
      return { lat, lon, angle, flipX: false, speedMps: 0, opacity: 1 };
    }
  ...
  _mobilePoseForRender(m, nowMs) {
    ...
    if (this.playbackMode) {
      ...
      return { lat, lon, angle, flipX, speedMps, opacity, reading: smp?.reading || null };
    }
  ...
  _requestTrailFxRAF() {
    ...
    const tick = () => {
      ...
      if (any && this.lastState) {
        this.drawOverlay(this.lastState, { nowMs, fromTrailFxTick: true });
        this._trailFxRAF = requestAnimationFrame(tick);
      }
    };
    this._trailFxRAF = requestAnimationFrame(tick);
  }
  ...
  _trailFxBoostFor(id, midFrac, nowMs) {
    ...
    // Breathing inside the band; phase-shifted by position so it "chases".
    const breathPeriodMs = 1200;
    const omega = (2 * Math.PI / breathPeriodMs);
    const localBreath = 0.70 + 0.30 * Math.sin((age * omega) - (midFrac * 2 * Math.PI * 1.25));

    // Overall amplitude decays out over the FX duration.
    const amp = 0.65 * (1 - t * 0.75);
    return 1.0 + (amp * band * localBreath);
  }
  ...
  _updatePersistedTrails(state) {
    ...
    for (const m of mobiles) {
      ...
      // Clean server trail points.
      const cleaned = [];
      for (const p of serverTrail) {
        const lat = Number(p?.lat);
        const lon = Number(p?.lon);
        if (!isFinite(lat) || !isFinite(lon)) continue;
        cleaned.push({
          lat,
          lon,
          t: (p && typeof p.t === "string") ? p.t : undefined,
          readings: (p && p.readings && typeof p.readings === "object") ? p.readings : undefined,
        });
      }

      // IMPORTANT: Do not replace our persisted trail with the server's current window.
      // Instead, append new points so the displayed path grows over time until reload.
      let nextTrail = Array.isArray(prev.trail) ? prev.trail : [];
      let appendedCount = 0;
      if (cleaned.length) {
        if (nextTrail.length === 0) {
          // Adopt the initial window.
          nextTrail = cleaned.slice();
          // If the bus is parked, immediately collapse the parked tail so we don't
          // start with a birds-nest from cached history.
          scrubReturnSpikes(nextTrail, { outM: 45, retM: 18, plateauM: 25, maxSpanMs: 180_000, maxScan: 2400 });
          collapseParkedSuffix(nextTrail, { tailN: 24, radiusM: 38, minPts: 14, minSpanMs: 120_000, maxScan: 5000, maxTravelM: 140 });
          appendedCount = nextTrail.length;
        } else {
          appendedCount = mergeByTimestamp(nextTrail, cleaned);
        }
      } else if (hasServerTrail) {
        // Defensive: hasServerTrail implies cleaned should have >=2, but keep behavior stable.
      }

      // When parked, aggressively compress the stationary tail to prevent birds-nests.
      // When not parked, keep every point (so depot/station movement is preserved).
      const scrubbed = scrubReturnSpikes(nextTrail, { outM: 45, retM: 18, plateauM: 25, maxSpanMs: 180_000, maxScan: 900 });
      const parkedCollapsed = collapseParkedSuffix(nextTrail, { tailN: 24, radiusM: 38, minPts: 14, minSpanMs: 120_000, maxScan: 5000, maxTravelM: 140 });

      // Parked marker debounce:
      // - Trust server-provided idle (`ghosted`) when available: if the backend says a
      //   vehicle is idle, pin the marker once and HOLD it (GPS drift can be larger
      //   than our purely geometric stationary heuristics).
      // - Otherwise, fall back to a motion-only stationary check.
      ...
      if (nextGhosted) {
        // Pin once when the backend declares idling (or if we somehow missed it).
        if (!nextPin || becameIdle) nextPin = tailMedianLatLon(nextTrail, 24);
      } else {
        // Not idle anymore: release immediately so real movement shows right away.
        nextPin = null;

        // Fallback: if it still looks stationary purely by motion, pin it.
      ...
      // Collapse near-collinear points at the tail to prevent unlimited growth from jitter.
      // Only touches the last few points and never removes corners/turns.
      if (nextTrail.length >= 3) {
        const a = nextTrail[nextTrail.length - 3];
        const b = nextTrail[nextTrail.length - 2];
        const c = nextTrail[nextTrail.length - 1];
        const d1 = haversineMeters(Number(a.lat), Number(a.lon), Number(b.lat), Number(b.lon));
        const d2 = haversineMeters(Number(b.lat), Number(b.lon), Number(c.lat), Number(c.lon));
        if (isFinite(d1) && isFinite(d2) && (d1 + d2) > 0) {
          const pd = perpDistMeters(a, b, c);
          if (isFinite(pd) && pd < collapseMeters) {
            // Replace middle point with end point (keeps shape but reduces redundancy).
            nextTrail.splice(nextTrail.length - 2, 1);
          }
        }
      }

      // Refresh metadata regardless of trail updates.
      const nextColor = m?.color;
      const metaChanged = (prev.color !== nextColor) || (prev.ghosted !== nextGhosted);

      if (appendedCount > 0 || metaChanged || parkedCollapsed || scrubbed || pinChanged) {
        this._persistedTrailById.set(id, { trail: nextTrail, color: nextColor, ghosted: nextGhosted, pin: nextPin });
        changed = true;

        // If we appended new trail points, mark a short-lived animation window.
        if (appendedCount > 0) {
  ...
  _playbackPointsKeyForState(state) {
    ...
    return `${revKey}|persist:${this._persistedTrailRev}`;
  }
  ...
  _playbackSampleForMobile(m, nowPerfMs) {
    ...
    const b = pts[idxHi];
    const a = pts[Math.max(0, idxHi - 1)];
    const dtMs = Math.max(1, (b.tMs - a.tMs));
    const u = clamp((t - a.tMs) / dtMs, 0, 1);
    const lat = a.lat + (b.lat - a.lat) * u;
    const lon = a.lon + (b.lon - a.lon) * u;
    const distM = haversineMeters(a.lat, a.lon, b.lat, b.lon);
    const speedMps = clamp(distM / Math.max(0.001, dtMs / 1000), 0, Number(this._traceRealMaxSpeedMps) || 20.0);

    // Heading + flip side + smoothed render angle (reuse caches).
    const w0 = latLonToWorld(a.lat, a.lon, this.zoom);
    const w1 = latLonToWorld(b.lat, b.lon, this.zoom);
    let dx = (w1.x - w0.x);
    let dy = (w1.y - w0.y);
    if (Math.abs(dx) < 0.0001 && Math.abs(dy) < 0.0001) {
      dx = 1e-3;
    ...
    // Historical reading: choose the closer endpoint's reading so the value matches the
    // discrete trail sample used for the dotted histogram trail.
    const idxPick = (u >= 0.5) ? idxHi : Math.max(0, idxHi - 1);
    const srcTrail = (this._persistedTrailById.get(id)?.trail || (Array.isArray(m?.trail) ? m.trail : []));
    const pRaw = Array.isArray(srcTrail) ? srcTrail[idxPick] : null;
    const reading = primaryReadingKeyedFromPoint(pRaw);

    return { lat, lon, angle: nextA, flipX: (side === "L"), speedMps, opacity: 1, reading };
  }
  ...
  _ensureOverlayStatic(state) {
    ...
    for (const f of fixed) {
        ...
        const emoji = f.emoji || "📍";
        const label = (f.name && f.name.length) ? `${f.id} (${f.name})` : f.id;
    ...
    const drawTrailFor = (m, alphaMul) => {
      ...
      for (const p of trail) {
        ...
        if (!isFinite(lat) || !isFinite(lon)) continue;
        const wpt = latLonToWorld(lat, lon, this.zoom);
        pts.push(worldToScreenFast(wpt.x, wpt.y));
        const pr = primaryReadingFromPoint(p);
        const base = safeHex(pr?.color || m.color);
        cols.push(isGhost ? dimHex(base, 0.65) : base);
      }
      ...
      // Solid "drawn path" underlay: subtle white halo (solid, not dashed).
      ctx.save();
      ctx.globalAlpha = (isSel2 ? 0.32 : 0.26) * alphaMul;
      ctx.strokeStyle = "rgba(255,255,255,0.80)";
      ctx.lineWidth = lw + (isSel2 ? 3.4 : 3.0);
      ctx.setLineDash([]); // solid
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.shadowColor = "rgba(255,255,255,0.45)";
      ctx.shadowBlur = isSel2 ? 8 : 7;
      ctx.beginPath();
      ...
      for (let i = 1; i < pts.length; i++) {
        ...
        const segColor = isGhost ? dimHex(segColor0, 0.65) : segColor0;

        // High-contrast under-stroke (solid, dark) so the colored dashes pop.
        ctx.save();
        ctx.globalAlpha = (isSel2 ? 0.92 : 0.80) * alphaMul;
        ctx.strokeStyle = "rgba(0,0,0,0.88)";
        ctx.lineWidth = lw + 2.6;
        ctx.setLineDash([]); // solid underlay for continuity
        ctx.lineCap = "round";
        ctx.lineJoin = "round";
        ctx.beginPath();
    ...
    // First pass A: non-selected trails without server history (persisted-only) underneath
    for (const m of mobiles) {
      if (selectedId && m.id === selectedId) continue;
      if (mobileHasServerTrail(m)) continue;
      drawTrailFor(m, selectedId ? 0.35 : 1.0);
    }
    // First pass B: non-selected trails with server history on top (most recent readings)
    for (const m of mobiles) {
      if (selectedId && m.id === selectedId) continue;
      if (!mobileHasServerTrail(m)) continue;
      drawTrailFor(m, selectedId ? 0.35 : 1.0);
    }
    // Second pass: selected trail on top at full strength
    if (selectedId) {
      const m = mobiles.find(x => x.id === selectedId);
      if (m) drawTrailFor(m, 1.0);
    }
  }
  ...
  drawOverlay(state, opts = {}) {
    ...
    const drawTrailFor = (m, alphaMul, toScreen) => {
      ...
      const key = keyFor("mobile", m && m.id != null ? m.id : "");
      const isSel = (this.selectedId === key);
      if (!!m.ghosted && !isSel) return false;
      const serverTrail = Array.isArray(m?.trail) ? m.trail : [];
      const hasServerTrail = serverTrail.length >= 2;
      const persistedTrail = id ? (this._persistedTrailById.get(id)?.trail || []) : [];
      const trail = (persistedTrail.length >= 2) ? persistedTrail : (hasServerTrail ? serverTrail : []);
      ...
      for (const p of trail) {
        ...
        if (!isFinite(lat) || !isFinite(lon)) continue;
        const wpt = latLonToWorld(lat, lon, this.zoom);
        pts.push((toScreen || this.worldToScreen.bind(this))(wpt.x, wpt.y));
        const pr = primaryReadingFromPoint(p);
        const base = safeHex(pr?.color || m.color);
        cols.push(isGhost ? dimHex(base, 0.65) : base);
      }
      ...
      // Solid "drawn path" underlay: subtle white halo (solid, not dashed).
      // The goal is to suggest a continuous path without turning it into a bright white road.
      ctx.save();
      // Restore the stronger underlay (two edits ago) so the path is clearly visible.
      ctx.globalAlpha = (isSelTrail ? 0.32 : 0.26) * alphaMul;
      ctx.strokeStyle = "rgba(255,255,255,0.80)";
      ctx.lineWidth = lw + (isSelTrail ? 3.4 : 3.0);
      ctx.setLineDash([]); // solid
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.shadowColor = "rgba(255,255,255,0.45)";
      ctx.shadowBlur = isSel ? 8 : 7;
      ctx.beginPath();
      ctx.moveTo(pts[0].x, pts[0].y);
      for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
      ctx.stroke();
      ...
      for (let i = 1; i < pts.length; i++) {
        ...
        // High-contrast under-stroke (solid, dark) so the colored dashes pop.
        ctx.save();
        ctx.globalAlpha = (isSelTrail ? 0.92 : 0.80) * alphaMul;
        ctx.strokeStyle = "rgba(0,0,0,0.88)";
        ctx.lineWidth = lw + 2.6;
        ctx.setLineDash([]); // solid underlay for continuity
        ctx.lineCap = "round";
        ...
        const mid = (pts.length > 1) ? ((i - 0.5) / (pts.length - 1)) : 0.0;
        const boost = this._trailFxBoostFor(id, mid, nowMs);
        ctx.globalAlpha = Math.min(1.0, alpha * boost);
        ctx.strokeStyle = segColor;
        ctx.lineWidth = lw;
        ctx.setLineDash(dash);
        ctx.lineCap = "round";
    ...
    if (!this.traceMode) {
      ...
      // First pass A: non-selected trails without server history underneath
      for (const m of mobiles) {
        if (selectedId && m.id === selectedId) continue;
        if (mobileHasServerTrail(m)) continue;
        drawTrailFor(m, selectedId ? 0.35 : 1.0, this.worldToScreen.bind(this));
      }
      // First pass B: non-selected trails with server history on top
      for (const m of mobiles) {
        if (selectedId && m.id === selectedId) continue;
        if (!mobileHasServerTrail(m)) continue;
        drawTrailFor(m, selectedId ? 0.35 : 1.0);
      ...
      if (selectedId) {
        const m = mobiles.find(x => x.id === selectedId);
        if (m) drawTrailFor(m, 1.0, this.worldToScreen.bind(this));
      }
    }
    ...
    for (const m of mobiles) {
      ...
      const by = sp.y + 18;
      ctx.fillStyle = "rgba(16, 20, 28, 0.82)";
      ctx.strokeStyle = safeHex(prColor || color);
      ctx.lineWidth = 1.8;
      roundRect(ctx, bx, by, bw, bh, 9);
      ctx.fill();
      ctx.stroke();
...
function renderLists(state, selectedId) {
  ...
  if (listFixedEl) listFixedEl.innerHTML = "";
  ...
  for (const m of mobiles) {
    ...
    el.className = "item" + (k === selectedId ? " selected" : "") + (m.ghosted ? " ghosted" : "");
    ...
    name.className = "name";
    name.textContent = m.name ? `${m.id} (${m.name})` : m.id;

    const pin = document.createElement("div");
    pin.className = "pin";
    pin.textContent = m.pinned ? (m.ghosted ? "pinned · idle" : "pinned") : (m.ghosted ? "idle" : "");
    ...
    for (const k of show) {
      ...
      const outC = m.ghosted ? dimHex(c, 0.65) : c;
    ...
    el.appendChild(row1);
    el.appendChild(row2);
    el.addEventListener("click", (e) => window.__selectSensor(k, { fitTrail: !!e.metaKey }));
    if (listMobileEl) listMobileEl.appendChild(el);
  }

  for (const f of fixed) {
    const el = document.createElement("div");
    const k = keyFor("fixed", f.id);
    el.className = "item" + (k === selectedId ? " selected" : "") + (f.ghosted ? " ghosted" : "");
    ...
    const name = document.createElement("div");
    name.className = "name";
    name.textContent = f.name ? `${f.id} (${f.name})` : f.id;
    ...
    const show = keys.slice(0, 3);
    for (const kk of show) {
      const v = readings[kk]?.value ?? "—";
      const c = safeHex(readings[kk]?.color);
      const outC = f.ghosted ? dimHex(c, 0.65) : c;
      const wrap = document.createElement("div");
      wrap.className = "reading";
      const kEl = document.createElement("span");
      kEl.className = "k";
    ...
    el.appendChild(row1);
    el.appendChild(row2);
    el.addEventListener("click", (e) => window.__selectSensor(k, { fitTrail: false }));
    if (listFixedEl) listFixedEl.appendChild(el);
  }
}
...
function main() {
  ...
  function saveViewSoon() {
    if (_viewSaveTimer) window.clearTimeout(_viewSaveTimer);
    _viewSaveTimer = window.setTimeout(() => {
      try {
  ...
  const updatePlaybackUi = () => {
    ...
    if (pbScrubEl && isFinite(b.minMs) && isFinite(b.maxMs) && b.maxMs > b.minMs) {
      const durS = Math.max(1, Math.floor((b.maxMs - b.minMs) / 1000));
      const tS = (tMs != null && isFinite(tMs)) ? Math.floor((tMs - b.minMs) / 1000) : durS;
      pbScrubEl.min = "0";
      pbScrubEl.max = String(durS);
      pbScrubEl.step = "1";
      pbScrubEl.disabled = false;
      if (!_pbScrubbing) pbScrubEl.value = String(clamp(tS, 0, durS));
    } else if (pbScrubEl) {
    ...
    if (pbPlayEl) pbPlayEl.textContent = map.getPlaybackPlaying() ? "Pause" : "Play";
  ...
  const playbackLoop = () => {
    ...
    if (map.getPlaybackPlaying() && isFinite(b.minMs) && isFinite(b.maxMs) && b.maxMs > b.minMs && tMs != null && isFinite(tMs)) {
      const next = tMs + dt * (map.getPlaybackSpeed() || 1.0);
      const clamped = clamp(next, b.minMs, b.maxMs);
      map.setPlaybackTimeMs(clamped);
      if (clamped >= b.maxMs - 0.5) map.setPlaybackPlaying(false);
    }

    // Basemap is static; redraw overlay only.
  ...
  if (traceEl) {
    ...
    if (traceEl.checked) {
      ...
      const b = map.getPlaybackBounds();
      if (isFinite(b.maxMs)) map.setPlaybackTimeMs(b.maxMs);
  ...
  if (pbPlayEl) {
    pbPlayEl.addEventListener("click", () => {
      ...
      if (map.getPlaybackTimeMs() == null && isFinite(b.maxMs)) map.setPlaybackTimeMs(b.maxMs);
      map.setPlaybackPlaying(!map.getPlaybackPlaying());
      updatePlaybackUi();
    });
  }
  ...
  if (pbScrubEl) {
    const applyScrub = () => {
      const b = map.getPlaybackBounds();
      if (!isFinite(b.minMs) || !isFinite(b.maxMs) || !(b.maxMs > b.minMs)) return;
      const sec = Number(pbScrubEl.value);
      if (!isFinite(sec)) return;
      const tMs = b.minMs + Math.round(sec) * 1000;
  ...
  async function tick() {
    ...
    try {
      ...
      if (map.playbackMode) {
        ...
        const tNow = map.getPlaybackTimeMs();
        const wasAtLive = (before.maxMs != null && tNow != null && isFinite(Number(before.maxMs)) && isFinite(Number(tNow)) && Math.abs(Number(before.maxMs) - Number(tNow)) <= 1100);
        map._ensurePlaybackPoints(st);
        const after = map.getPlaybackBounds();
        if (wasAtLive && after.maxMs != null && isFinite(Number(after.maxMs))) map.setPlaybackTimeMs(Number(after.maxMs));
      }
