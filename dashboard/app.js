...
function main() {
  ...
  let _pbResumeAfterScrub = false;
  let _pbInertiaVel = 0; // slider units per ms
  let _pbLastScrubPos = 0;
  let _pbLastScrubTime = 0;
  let _pbPlayEase = 0; // 0..1 ramp for global start/stop easing
  ...
  const playbackLoop = () => {
    ...
    if (didMarkerInertia) {
      ...
      if (clamped >= b.maxMs - 0.5) {
        ...
        map._playbackLiveFollow = true; // Auto-transition to LIVE follow-tail
        didAdvanceTime = true;
        tMs = b.maxMs;
      }
    }
    ...
    if (!didMarkerInertia && !_pbScrubbing && Math.abs(_pbInertiaVel) > 1e-5) {
    ...
    if (!_pbScrubbing && _pbResumeAfterScrub && Math.abs(_pbInertiaVel) <= 1e-5) {
    ...
    if (map.getPlaybackPlaying() || markerInertiaActive || Math.abs(_pbInertiaVel) > 1e-5) {
      _pbRAF = requestAnimationFrame(playbackLoop);
    } else {
      // Reset perf accumulator so dt doesn't jump if we resume later.
      _pbLastPerf = 0;
    }
  };
  ...
  if (pbPlayEl) {
    pbPlayEl.addEventListener("click", () => {
      ...
      // If we're at the end (LIVE), stay pinned to end-of-data.
      const atEnd = map.isPlaybackAtEnd(100); // use stricter epsilon for entering live mode
      if (atEnd) {
        map._playbackLiveFollow = true;
        map.setPlaybackTimeMs(b.maxMs);
      } else {
        // Resume from wherever the user scrubbed, but if at the beginning or invalid, always jump to the end.
        const cur = map.getPlaybackTimeMs();
        if (cur == null || !isFinite(cur) || cur === b.minMs) {
          map.setPlaybackTimeMs(b.maxMs);
        }
        // Leaving LIVE follow-tail: the user is explicitly playing historical time forward.
